...
# =========================
SCENARIO_MASTER_FILE = os.path.join(os.getcwd(), "Eastspring_Scenarios.xlsx")
...
def run_upload_analysis_job(job_id: str, username: str, doc_files_meta: list, audio_paths: list):
    print(f"[JOB {job_id}] START user={username} docs={len(doc_files_meta)} audios={len(audio_paths)}")
    ...
    try:
        ...
        for idx, item in enumerate(doc_files_meta, start=1):
            ...
            print(f"[JOB {job_id}] DOC {idx}/{len(doc_files_meta)} START {filename}")

            from io import BytesIO
            fs = BytesIO(data_bytes)

            class _Tmp:
                def read(self): return fs.getvalue()

            text = extract_text_from_pdf(_Tmp()) if ext == "pdf" else extract_text_from_docx(_Tmp())
            if not (text or "").strip():
                print(f"[JOB {job_id}] DOC SKIP (no text) {filename}")
                continue

            data = analyze_single_text_for_ui(text, os.path.basename(filename))
            print(f"[JOB {job_id}] DOC DONE {filename} success={data.get('success')} sentiment={data.get('sentiment')}")
        ...
        for idx, file_path in enumerate(audio_paths, start=1):
            ...
            print(f"[JOB {job_id}] AUDIO {idx}/{len(audio_paths)} START {filename}")

            try:
                label, score, details, raw_data = analyze_sentiment_from_wav(file_path)
                print(f"[JOB {job_id}] AUDIO DONE {filename} label={label} score={score}")
                ...
                save_sentiment_to_db(
                    username=username,   # ✅ correct (do NOT use session here)
                    source_type="audio",
                ...
                # IMPORTANT: don’t silently continue — log the error
                print(f"[JOB {job_id}] AUDIO ERROR {filename}: {e}")
                continue

        # ✅ mark job done
        with JOBS_LOCK:
            JOBS[job_id] = {"status": "done", "message": "Sentiment result is ready ✅", "username": username}

        print(f"[JOB {job_id}] MARKED DONE ✅")

        # ✅ send push AFTER done
        ok, msg = send_push_to_user(
            username,
            "Sentiment ready ✅",
            "Your sentiment result is ready. Click to view."
        )
        print(f"[JOB {job_id}] PUSH: ok={ok} msg={msg}")

    except Exception as e:
        print(f"[JOB {job_id}] PUSH ERROR (ignored): {e}")
        ...
        try:
           ...
           print(f"[JOB {job_id}] PUSH FAIL NOTICE ERROR (ignored): {e2}")
    ...
    try:
        ...
        cur.execute(
            ...
            (username, source_type, filename, text_input, sentiment, confidence,
             tone, scenario_id, scenario_title, explanation, transcript)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                username, source_type, filename, text_input, sentiment, confidence,
...
# Wrapper so /upload can keep using 4 return values
...
def load_history_rows():
    ...
    wb = load_workbook(EXCEL_PATH)
    ws = wb["Sentiment_Results"]

    for row_vals in ws.iter_rows(min_row=2, values_only=True):
        if not row_vals or all(c is None for c in row_vals):
            continue

        vals = list(row_vals) + [None] * (10 - len(row_vals))
        first_cell = vals[0]

        # Detect OLD format by checking if first cell looks like a .wav path
        is_old_format = isinstance(first_cell, str) and ".wav" in first_cell.lower()

        if is_old_format:
            # ---------- OLD FORMAT ----------
            audio_path = str(vals[0]) if vals[0] else ""
            dt_val = None
            comment = ""

            # Try guess date/time from audio modified time
            if audio_path:
                norm_path = audio_path.replace("\\", "/")
                candidate = norm_path if os.path.isabs(norm_path) else os.path.join(PROJECT_ROOT, norm_path)

                try:
                    if os.path.exists(candidate):
                        dt_val = datetime.fromtimestamp(os.path.getmtime(candidate))
                except Exception:
                    dt_val = None

            audio_file = os.path.basename(audio_path.replace("\\", "/")) if audio_path else ""
            ...
            explanation = vals[5]
            scenario_id = vals[6]
            scenario_title = vals[7]
            transcript = vals[8]
            comment = vals[9]
            
        # ✅ ALWAYS compute file_type here (works for BOTH old and new)
        file_type = detect_file_type(audio_file)
        source_type = detect_source_type(audio_file)


        # ✅ Summary: short explanation
        short_summary = re.sub(r"\s+", " ", str(explanation or "")).strip()
...
def delete_history_entry(row_id: int):
    ...
    # last row is newest
    excel_row_number = ws.max_row - row_id

    if excel_row_number < 2 or excel_row_number > ws.max_row:
        return

    ws.delete_rows(excel_row_number)
    wb.save(EXCEL_PATH)
...
def update_history_entry(row_id: int, updates: dict):
    ...
    # UI is newest-first, Excel is oldest-first
    # UI 0 = newest = last Excel data row
    excel_row_number = (ws.max_row - row_id)

    # Safety check (must never touch header row 1)
    if excel_row_number < 2 or excel_row_number > ws.max_row:
        return

    # Update summary (explanation column 6)
    if "summary" in updates:
        ws.cell(row=excel_row_number, column=6).value = updates["summary"]

    # Update comment (column 10)
    if "comment" in updates:
        ws.cell(row=excel_row_number, column=10).value = updates["comment"]
